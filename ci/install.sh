...
help() {
  cat <<'EOF'
Install a binary release of Slipway hosted on GitHub
...
crate=slipway
url=https://github.com/slipwayhq/slipway
...
if [ -z "${tag-}" ]; then
  tag=$(
    download https://api.github.com/repos/slipwayhq/slipway/releases/latest - |
    grep tag_name |
    cut -d'"' -f4
  )
fi
