...
build target="debug": (build-components target) (build-src target)
...
build-components target="debug": && assemble-test-components
  cd src-components && cargo build {{ if target == "release" { "--release" } else { "" } }}
...
assemble-test-components:
  ...
  cp src-components/target/wasm32-wasi/debug/slipway_test_component.wasm test-components/slipway_test_component/slipway_component.wasm
  ...
  cp src-components/target/wasm32-wasi/debug/slipway_test_component.wasm test-components/slipway_test_component_json_schema/slipway_component.wasm
